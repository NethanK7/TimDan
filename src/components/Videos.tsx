      ...
      <div className="mx-auto max-w-[1400px] px-5 sm:px-8">
        <div className="mb-14 flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="h-section max-w-[13ch]">
          ...
          {channel && (
            <Reveal delay={0.1}>
              <div className="flex items-center gap-5 rounded-lg border border-line bg-raise/40 px-6 py-4">
                <div>
        ...
        {videos.length > 0 ? (
          ...
          <Reveal>
            <div className="overflow-hidden rounded-lg border border-line bg-raise/30">
              ...
              {error && (
                <p className="border-b border-line bg-ink px-8 py-4 font-mono text-[0.72rem] text-gold">
                  {error}
                </p>
              )}
        ...
        {/* ── podcast band ───────────────────────── */}
        <div className="mt-28 overflow-hidden rounded-lg border border-line bg-gradient-to-br from-raise/70 via-ink-2 to-ink">
          <div className="grid gap-10 p-9 sm:p-14 lg:grid-cols-12 lg:items-center">
            <div className="lg:col-span-7">
              <Reveal>
                <p className="eyebrow mb-5">{podcast.eyebrow}</p>
              </Reveal>
              <h3 className="h-section mb-5">
                <span className="text-gold-soft">{podcast.name}</span>
              </h3>
              <p className="h-card mb-6 text-bone/85">{podcast.tagline}</p>
                ...
                <div className="flex flex-col gap-3">
                  {podcast.links.map((l) => (
                    <a
                      ...
                      className="group flex items-center justify-between gap-12 rounded-lg border border-line bg-ink/60 px-6 py-4 transition-colors duration-500 hover:border-gold/50"
                    >
                      <span className="text-sm text-bone">{l.label}</span>
                      <span className="text-dim transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:translate-x-1 group-hover:text-gold">
                        &rarr;
                      </span>
                    </a>
                  ))}
